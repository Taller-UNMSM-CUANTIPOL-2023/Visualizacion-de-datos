...
```{r warning=FALSE}
df_lapop <- read_stata("https://github.com/Taller-UNMSM-CUANTIPOL-2023/Visualizacion-de-datos/raw/main/PER_2021_LAPOP.dta") |> 
...
bp3 <- #boxplot(df_lapop$m1,col = "pink") # boxplot basico de R
  
  ggplot(df_pre, aes(x = "", y = percepcion, color = percepcion)) +
...
```{r}
# HIST:
h <- ggplot(df_lapop, aes(x = confi_ins, color = "")) + 
  geom_histogram(breaks = seq(105), fill = "white", 
...
```{r}
df_lapop <- df_lapop[!is.na(df_lapop$sex),] #Eliminando casos perdidos


bp2 <- ggplot(df_lapop, aes(x = sex, y = confi_ins, color = sex)) + 
